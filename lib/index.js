...
class Month {
  ...
  of(year) { // calculate the date and return it
    ...
    // this is the number of days to the first day of the desired day (0-6).
    ...
    // now we calculate if we need to add more days to get to the desired week.
    ...
    // I'm going to just check if we ended up in the next month.
    // if so, then I'll backup a week to be on the right day in the right month.
    if (date.getMonth() !== this.month) {
      // then we went too far, so, backup :)
      // backup 7 days (1 week) to get to the same day in the month's last week.
